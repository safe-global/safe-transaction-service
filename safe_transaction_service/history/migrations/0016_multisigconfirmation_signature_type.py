...
def add_signature_type(apps, schema_editor):
    ...
    for multisig_confirmation in MultisigConfirmation.objects.all():
        if not multisig_confirmation.signature:  # It's an approvedHash
            multisig_confirmation.signature = SafeSignatureApprovedHash.build_for_owner(
                multisig_confirmation.owner, multisig_confirmation.multisig_transaction_hash).export_signature()
            ...
            for safe_signature in SafeSignature.parse_signature(multisig_confirmation.signature,
    ...
    for multisig_tx in MultisigTransaction.objects.exclude(signatures=None):
        for safe_signature in SafeSignature.parse_signature(multisig_tx.signatures.tobytes(),
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.AddField(
            ...
            name='signature_type',
            field=models.PositiveSmallIntegerField(choices=[(0, 'CONTRACT_SIGNATURE'),
                                                            (1, 'APPROVED_HASH'),
                                                            (2, 'EOA'),
