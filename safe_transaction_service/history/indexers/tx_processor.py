...
class SafeTxProcessor(TxProcessor):
    ...
    def get_last_safe_status_for_address(self, address: ChecksumAddress) -> Optional[SafeStatus]:
    ...
    def __process_decoded_transaction(self, internal_tx_decoded: InternalTxDecoded) -> bool:
        ...
        if function_name == 'setup' and contract_address != NULL_ADDRESS:
            ...
            if not safe_status:
                ...
                safe_status.threshold = arguments['_threshold'] or safe_status.threshold  # Event doesn't have threshold
                ...
                ethereum_tx = internal_tx.ethereum_tx
                if 'module' in arguments:  # L2 Safe with event SafeModuleTransaction indexed using events
