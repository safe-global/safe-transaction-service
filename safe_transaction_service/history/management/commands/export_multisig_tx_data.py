    ...
    def handle(self, *args, **options):
        ...
        if count:
            with open(file_name, 'w') as f:
                ...
                f.write('|'.join(['Execution date', 'tx-hash', 'safe-address', 'gas_price', 'gas_limit', 'gas_used',
                                  'to', 'failed', 'origin', 'decodedData']) + '\n')
                for m in queryset.select_related('ethereum_tx__block'):
                    ethereum_tx = m.ethereum_tx
                    f.write('|'.join([str(m.execution_date), m.ethereum_tx_id, m.safe, str(ethereum_tx.gas_price),
                                      str(m.ethereum_tx.gas), str(m.ethereum_tx.gas_used),
                                      m.to, str(m.failed), m.origin,
