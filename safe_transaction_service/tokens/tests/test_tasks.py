...
class TestTasks(TestCase):
    ...
    def test_calculate_token_eth_price_task(self, timezone_now_mock: MagicMock, get_token_eth_value_mock: MagicMock):
        ...
        self.assertEqual(calculate_token_eth_price_task.delay(random_token_address, random_redis_key).result, expected)

        # Check caching works even if we change the token_address
        another_token_address = Account.create().address
        self.assertEqual(calculate_token_eth_price_task.delay(another_token_address, random_redis_key).result, expected)
    ...
    def test_calculate_token_eth_price_task_without_mock(self):
        ...
        random_redis_key = Account.create().address
        eth_value_with_timestamp = calculate_token_eth_price_task(dai_address, random_redis_key)
        self.assertGreater(eth_value_with_timestamp.eth_value, 0.)

        pool_together_address = '0x334cBb5858417Aee161B53Ee0D5349cCF54514CF'
        random_redis_key = Account.create().address
        eth_value_with_timestamp = calculate_token_eth_price_task(pool_together_address, random_redis_key)
        self.assertGreater(eth_value_with_timestamp.eth_value, 0.)

        random_token_address = Account.create().address
        random_redis_key = Account.create().address
        eth_value_with_timestamp = calculate_token_eth_price_task(random_token_address, random_redis_key)
        self.assertEqual(eth_value_with_timestamp.eth_value, 0.)
