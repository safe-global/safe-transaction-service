...
from ...safe_messages.tests.mocks import get_eip712_payload_mock
...
class TestAccountAbstractionViews(SafeTestCaseMixin, APITestCase):
    def test_safe_operation_view(self):
        ...
        safe_operation = factories.SafeOperationFactory(
            user_operation__sender=self.deploy_test_safe().address
        )
        response = self.client.get(
            reverse(
    ...
    def test_safe_operations_view(self):
        ...
        response = self.client.get(
            reverse("v1:safe_operations:message", args=(safe_operation.hash,))
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertDictEqual(
            response.json(),
            {
                "created": datetime_to_str(safe_operation.created),
                "modified": datetime_to_str(safe_operation.modified),
                "safe": safe_operation.safe,
                "messageHash": safe_operation.hash,
                "message": safe_operation.message,
                "proposedBy": safe_operation.proposed_by,
                "safeAppId": safe_operation.safe_app_id,
                "preparedSignature": safe_operation_confirmation.signature,
                "confirmations": [
                    {
                        "created": datetime_to_str(safe_operation_confirmation.created),
                        "modified": datetime_to_str(
                            safe_operation_confirmation.modified
                        ),
                        "owner": safe_operation_confirmation.owner,
                        "signature": safe_operation_confirmation.signature,
                        "signatureType": "EOA",
    ...
    def test_safe_operation_create_view(self, get_owners_mock: MagicMock):
    ...
    def test_safe_message_view_v1_1_1(self):
        ...
        response = self.client.get(
            reverse(
                "v1:account_abstraction:safe-operation",
                args=(safe_message.message_hash,),
            )
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
