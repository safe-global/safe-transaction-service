...
class QueueService:
    EXCHANGE_NAME = settings.EVENTS_QUEUE_RABBITMQ_EXCHANGE_NAME
    ...
    def send_event(
        ...
        :param event: Dict with the payload of the event
        """
...
class MockedQueueService:
    ...
    def send_event(self, event: Dict[str, Any]):
        logger.warning("MockedQueueService: Not sending event with payload %s", event)
